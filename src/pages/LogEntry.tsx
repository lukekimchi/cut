  ...
  return (
    <div className="page log-page">
      ...
      <div className="date-nav">
        <button className="date-nav-arrow" onClick={goBack}>‹</button>
        <span className="date-nav-label">{displayDate}</span>
        <button className="date-nav-arrow" onClick={goForward} disabled={date >= today}>›</button>
      </div>

      {/* ── Sliding content ── */}
      <div key={date} className={`log-slide ${slideDir === 'left' ? 'slide-from-right' : 'slide-from-left'}`}>
        {loading ? (
          <div className="page-loading" style={{ paddingTop: 32 }}>Loading…</div>
        ) : (
          <>
            {/* Body Weight + Calories Burned */}
            <div className="daily-stats-row">
              <div
                className={`daily-stat-card${editingWeight ? ' editing' : ''}`}
                onClick={() => !editingWeight && setEditingWeight(true)}
                role="button"
                tabIndex={0}
                onKeyDown={e => e.key === 'Enter' && !editingWeight && setEditingWeight(true)}
              >
                {editingWeight ? (
                  <form onSubmit={handleSaveWeight} onClick={e => e.stopPropagation()} className="daily-stat-edit">
                    <input
                      type="number"
                      className="input daily-stat-input"
                      value={weight}
                      ...
                      autoFocus
                    />
                    <div className="daily-stat-actions">
                      <button type="button" className="btn btn-secondary" onClick={() => setEditingWeight(false)}>✕</button>
                  ...
                  <>
                    <span className="daily-stat-value">{weight || '—'}</span>
                    <span className="daily-stat-unit">kg</span>
                  </>
                )}
              </div>

              <div
                className={`daily-stat-card${editingExpenditure ? ' editing' : ''}`}
                onClick={() => !editingExpenditure && setEditingExpenditure(true)}
                role="button"
                tabIndex={0}
                onKeyDown={e => e.key === 'Enter' && !editingExpenditure && setEditingExpenditure(true)}
              >
                {editingExpenditure ? (
                  <form onSubmit={handleSaveExpenditure} onClick={e => e.stopPropagation()} className="daily-stat-edit">
                    <input
                      type="number"
                      className="input daily-stat-input"
                    ...
                    />
                    <div className="daily-stat-actions">
                      <button type="button" className="btn btn-secondary" onClick={() => setEditingExpenditure(false)}>✕</button>
                      <button type="submit" className="btn btn-primary" disabled={savingExpenditure}>
                        {savingExpenditure ? '…' : '✓'}
                      </button>
                    </div>
                  </form>
                  ...
                  <>
                    <span className="daily-stat-value">
                      {caloriesBurned ? Number(caloriesBurned).toLocaleString() : '—'}
                    </span>
                    <span className="daily-stat-unit">kcal burned</span>
                  </>
                )}
              </div>
            </div>
            ...
            {/* Food Log */}
            <h3 className="section-title" style={{ marginTop: 4 }}>Food Log</h3>
      ...
      {!showForm && (
        <button className="fab" onClick={() => setShowForm(true)} aria-label="Add meal">
          + Add Meal
        </button>
      )}
    </div>
